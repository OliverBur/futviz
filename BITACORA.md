...
### 2026-07-26 — ejecutado el plan de pulido de la sesión anterior

Se atacaron, en orden, los 9 puntos del plan dejado la noche anterior ("mañana te digo que le des a eso" → "listo, ataca uno por uno"). Todo en `web/site_utils.py` salvo que se indique otra cosa; verificado con `python web/build.py` sin errores tras cada tramo y revisado visualmente (landing, `equipos.html`, `jugadores.html`, una página de chart) antes de cerrar.

1. **Foco de teclado**: las hub-cards de la landing ya no dependen solo del cambio de fondo/sombra en `:focus-visible` — se agregó un `outline` explícito (2px, color `--color-primary`, offset 3px).
2. **Google Fonts unificado**: se creó una sola constante `FONT_LINKS` (un `<link>` con los pesos 400–800 de Inter) reusada en las tres plantillas (`PAGE_TEMPLATE`, `INDEX_TEMPLATE`, `SECTION_PAGE_TEMPLATE`) — antes cada una cargaba su propio `<link>` con pesos ligeramente distintos, y las páginas de chart ni siquiera cargaban Inter.
3. **Acento visible en reposo**: el ícono de cada hub-card pasó de flotar suelto a vivir dentro de un "chip" con fondo de color (antes el verde-menta solo aparecía en `:hover`).
4. **Diferenciación Equipos/Jugadores**: se resolvió junto con el punto 3 — el chip de Equipos usa fondo verde-menta (`--color-accent-soft`) y el de Jugadores fondo azul petróleo tenue (`rgba(45,86,97,.10)`), con el ícono siempre en `--color-primary` para mantener buen contraste; el borde de la card de Equipos en reposo también se tiñe levemente de verde para reforzar la distinción. Sigue usando solo los 5 colores fijos de la paleta, no se inventó ningún color nuevo.
5. **Firma de marca**: se agregó un monograma de texto "FV" (badge cuadrado, fondo `--color-primary`) arriba del título "FutViz", y un pie de página chico ("Datos: fbref (equipos) & Understat (jugadores) · Temporada 2025/2026") debajo de las cards — el usuario aclaró que el logo real como imagen lo va a traer después, por ahora es solo texto.
6. **Composición de la landing**: se agregaron dos `radial-gradient` muy sutiles (12–18% opacidad, mismos colores de marca) al fondo del `body`, para que pantallas anchas no se sientan vacías sin convertir la portada en un "dashboard".
7 y 8 (se hicieron juntos, no tenía sentido rehacer el header dos veces). **Header de las páginas de chart + migración de paleta**: `PAGE_TEMPLATE` dejó de usar el tema `INK` de matplotlib (Segoe UI, colores del EDA) y ahora comparte `BRAND_ROOT_CSS` con el resto del sitio. El header pasó de ser un link suelto a un breadcrumb real con marca ("FV FutViz / {Sección}"), igual en estructura a las otras dos capas. El área del gráfico (Plotly o imagen PNG) quedó enmarcada en una tarjeta blanca con borde, sin tocar los colores del gráfico en sí (esos siguen siendo del tema `INK`, a propósito — es contenido, no chrome).
9. **Grillas de `equipos.html`/`jugadores.html`**: dejaron de ser cards de puro texto. Se agregó el campo `kind` a `ChartPage` (`scatter | radar | bar | box`, default `scatter`) seteado explícitamente en cada `ChartPage(...)` de `web/charts/teams.py` y `web/charts/players.py` según el tipo real de gráfico (radar: los 2 de perfil de liga; bar: ranking de porterías; box: paridad, edad, disciplina, y los 2 boxplots de jugadores; el resto quedó en scatter, el default), con un ícono SVG distinto por tipo (`CHART_ICONS`) mostrado en un chip a la izquierda de cada card. También se agregó el breadcrumb real "FV FutViz / {Sección}" en el header, igual que en las páginas de chart. **Fuera de alcance a propósito** (el usuario lo dejó explícitamente para después): subagrupar la grilla por categoría (ej. "Portería", "Disciplina") dentro de cada sección.
- Limpieza de paso: se sacó el import muerto de `INK` en `site_utils.py` (ya no se usa ahí tras el punto 8).

### 2026-07-25 (noche) — landing como hub de 2 cards + plan de pulido pendiente

- Rediseño estructural de `index.html`: pasó de índice plano con los 14 charts visibles a una portada minimalista (título "FutViz" + descripción + 2 cards grandes "Equipos"/"Jugadores", cada una con ícono SVG, descripción de una línea y contador de gráficas). El contenido que antes vivía en el índice se movió a dos páginas de sección nuevas (`web/dist/equipos.html`, `web/dist/jugadores.html`), que conservan la grilla de links a cada chart con la misma paleta de marca. Cambios en `web/site_utils.py` (`SECTION_META`, `INDEX_TEMPLATE`, `SECTION_PAGE_TEMPLATE`, `write_section_pages()`) y `web/build.py`.
- De paso se quitó el título/subtítulo duplicado en las páginas de chart individuales (`PAGE_TEMPLATE`): el usuario notó en capturas que el header de la página repetía literalmente el título/subtítulo que ya trae el propio gráfico (Plotly o PNG) vía `fig.update_layout(title=...)` — se sacó el `<h1>`/`.subtitle` del shell, queda solo el link de "Volver a {sección}".
- **Revisión crítica pedida explícitamente** ("sé crítico, quiero que se vea realmente bien"). Se identificaron 11 puntos débiles del rediseño (landing, páginas de sección, páginas de chart, sistema tipográfico). El usuario aprobó atacar 9 de los 11 en la próxima sesión (deja fuera por ahora: agrupar/subcategorizar la grilla de charts dentro de cada sección, y el ícono/isotipo real de FutViz — por ahora solo texto, el logo como imagen lo va a traer él).

**Plan de acción para la próxima sesión, de más fácil a más difícil** (todo en `web/site_utils.py` salvo que se indique otra cosa):

1. **Foco de teclado en las cards de la landing** — `:focus-visible` hoy solo cambia fondo/sombra y quita el outline; agregar un anillo de foco visible explícito (`outline` con offset, color de marca) para accesibilidad real.
2. **Un solo `<link>` de Google Fonts, reusado en las 3 capas** — hoy se carga por separado en `INDEX_TEMPLATE` y `SECTION_PAGE_TEMPLATE` (con pesos ligeramente distintos) y ni siquiera llega a `PAGE_TEMPLATE` (páginas de chart, que se quedaron en Segoe UI vía el tema `INK` de `viz_theme.py`) — inconsistencia de tipografía entre landing → sección → chart.
3. **Acento visible en reposo en las hub-cards** — ahora mismo el verde-menta solo aparece en `:hover`; en reposo las cards son puro blanco+borde azul, la paleta no se nota hasta que interactúas. Poner un chip con fondo `--color-accent-soft` detrás del ícono en vez de ícono suelto.
4. **Diferenciar Equipos de Jugadores visualmente** — mismo ícono, mismo verde, mismo borde en las dos cards; a simple vista son intercambiables salvo el texto. Buscar una variación sutil (tono de borde, fondo de ícono) sin salirse de la paleta de 5 colores fija.
5. **Firma de marca / remate de página** — "FutViz" es solo texto sin ancla (sin mark, sin footer con fuente de datos / última actualización). Placeholder de texto por ahora (el usuario va a traer un logo real después) — un pie de página simple alcanza para que la portada no se sienta inacabada.
6. **Composición de la landing en pantallas grandes** — todo centrado en un bloque de 900px sobre fondo plano #EFEDF7 se ve muy vacío en monitores anchos. Evaluar textura/gradiente sutil o desplazar el bloque del centro vertical exacto.
7. **Header de las páginas de chart individuales** — quedó como un link flotando arriba a la izquierda sin tratamiento visual, desconectado del resto del sitio tras sacarle el título/subtítulo. Necesita algo de identidad (aunque sea mínima) para no sentirse "abandonado".
8. **Migrar páginas de chart a la paleta de marca** — hoy usan el tema `INK` de matplotlib (colores/tipografía del EDA), no las variables `--color-primary`/`--color-bg`/etc. de la landing/secciones. Es el salto más grande de estilo dentro del sitio: pasar de una portada con identidad fuerte a una página de gráfico que parece de otro proyecto.
9. **Rediseñar las cards de la grilla en `equipos.html`/`jugadores.html`** — hoy son, literal, el índice de links viejo partido en dos (solo título+subtítulo de texto plano, sin preview ni ícono por tipo de gráfico). Es el punto más grande: implica tocar `web/charts/{teams,players}.py` si se quiere algo más que texto (ej. ícono por tipo de chart) y sumar breadcrumb real (`FutViz > Equipos`, hoy solo hay "← Volver").

Queda pendiente confirmación del usuario mañana para arrancar (dijo "mañana te digo que le des a eso").
